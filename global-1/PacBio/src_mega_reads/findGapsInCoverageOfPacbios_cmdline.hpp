...
public:
  ...
  bool                           input_file_given;
  int                            max_gap_overlap_arg;
  bool                           max_gap_overlap_given;
  int                            min_ovl_implied_vs_gap_end_arg;
  bool                           min_ovl_implied_vs_gap_end_given;
  int                            min_match_len_for_implied_match_arg;
  bool                           min_match_len_for_implied_match_given;

  enum {
    START_OPT = 1000,
    MAX_GAP_OVERLAP_OPT,
    MIN_OVL_IMPLIED_VS_GAP_END_OPT,
    MIN_MATCH_LEN_FOR_IMPLIED_MATCH_OPT
  };

  cmdline_parse() :
    input_file_arg(""), input_file_given(false),
    max_gap_overlap_arg((int)70), max_gap_overlap_given(false),
    min_ovl_implied_vs_gap_end_arg((int)100), min_ovl_implied_vs_gap_end_given(false),
    min_match_len_for_implied_match_arg((int)30), min_match_len_for_implied_match_given(false)
  { }
  ...
  void parse(int argc, char* argv[]) {
    static struct option long_options[] = {
      {"input-file", 1, 0, 'f'},
      {"max-gap-overlap", 1, 0, MAX_GAP_OVERLAP_OPT},
      {"min-ovl-implied-vs-gap-end", 1, 0, MIN_OVL_IMPLIED_VS_GAP_END_OPT},
      {"min-match-len-for-implied-match", 1, 0, MIN_MATCH_LEN_FOR_IMPLIED_MATCH_OPT},
      {"help", 0, 0, 'h'},
      {"usage", 0, 0, 'U'},
      {"version", 0, 0, 'V'},
      {0, 0, 0, 0}
    };
    static const char *short_options = "hVUf:";
...
#define CHECK_ERR(type,val,which) if(!err.empty()) { ::std::cerr << "Invalid " #type " '" << val << "' for [" which "]: " << err << "\n"; exit(1); }
    while(true) {
      ...
      case 'f':
        ...
        input_file_arg = optarg;
        break;
      case MAX_GAP_OVERLAP_OPT:
        max_gap_overlap_given = true;
        max_gap_overlap_arg = conv_int<int>((const char*)optarg, err, false);
        CHECK_ERR(int_t, optarg, "    --max-gap-overlap=int")
        break;
      case MIN_OVL_IMPLIED_VS_GAP_END_OPT:
        min_ovl_implied_vs_gap_end_given = true;
        min_ovl_implied_vs_gap_end_arg = conv_int<int>((const char*)optarg, err, false);
        CHECK_ERR(int_t, optarg, "    --min-ovl-implied-vs-gap-end=int")
        break;
      case MIN_MATCH_LEN_FOR_IMPLIED_MATCH_OPT:
        min_match_len_for_implied_match_given = true;
        min_match_len_for_implied_match_arg = conv_int<int>((const char*)optarg, err, false);
        CHECK_ERR(int_t, optarg, "    --min-match-len-for-implied-match=int")
        break;
      }
    }
    ...
    if(!input_file_given)
      error("[-f, --input-file=path] required switch");
    ...
    // Check access rights
    if(access(input_file_arg, R_OK)) {
      err = "Switch -f, --input-file=path, access right (read) failed for file '";
      ((err += input_file_arg) += "': ") += strerror(errno);
      error(err.c_str());
    }
  }
  ...
  static const char * help() { return
    "Find were we shouldn't join sub-mega reads for given pacbio reads.\n\n\n\n"
    "Options (default value in (), *required):\n"
    " -f, --input-file=path                   *Pacbio read-super-read match file\n"
    "     --max-gap-overlap=int                Shorter matches between good-overlap regions are considered a gap (70)\n"
    "     --min-ovl-implied-vs-gap-end=int     Implied overlaps which overlap good match regions by less than this are not processed (100)\n"
    "     --min-match-len-for-implied-match=int\n                                          Will not use implied matches of matches whose actual match is shorter than this (30)\n"
    " -U, --usage                              Usage\n"
    " -h, --help                               This message\n"
    " -V, --version                            Version";
  }
...
#define PACKAGE_VERSION "0.0.0"
  ...
  void dump(::std::ostream &os = std::cout) {
    os << "input_file_given:" << input_file_given << " input_file_arg:" << input_file_arg << "\n";
    os << "max_gap_overlap_given:" << max_gap_overlap_given << " max_gap_overlap_arg:" << max_gap_overlap_arg << "\n";
    os << "min_ovl_implied_vs_gap_end_given:" << min_ovl_implied_vs_gap_end_given << " min_ovl_implied_vs_gap_end_arg:" << min_ovl_implied_vs_gap_end_arg << "\n";
    os << "min_match_len_for_implied_match_given:" << min_match_len_for_implied_match_given << " min_match_len_for_implied_match_arg:" << min_match_len_for_implied_match_arg << "\n";
  }
};
