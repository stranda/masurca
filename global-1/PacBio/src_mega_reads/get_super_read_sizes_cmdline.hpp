...
public:
  const char *                   kunitig_lenghts_file_arg;
  bool                           kunitig_lenghts_file_given;
  const char *                   super_reads_file_arg;
  bool                           super_reads_file_given;
  ...
  cmdline_parse() :
    kunitig_lenghts_file_arg(""), kunitig_lenghts_file_given(false),
    super_reads_file_arg(""), super_reads_file_given(false)
  { }
  ...
  void parse(int argc, char* argv[]) {
    static struct option long_options[] = {
      {"kunitig_lenghts_file", 1, 0, 'k'},
      {"super_reads_file", 1, 0, 's'},
      {"help", 0, 0, 'h'},
      {"usage", 0, 0, 'U'},
      {"version", 0, 0, 'V'},
      {0, 0, 0, 0}
    };
    static const char *short_options = "hVUk:s:";
...
#define CHECK_ERR(type,val,which) if(!err.empty()) { ::std::cerr << "Invalid " #type " '" << val << "' for [" which "]: " << err << "\n"; exit(1); }
    while(true) {
      ...
      case '?':
        ...
        exit(1);
      case 'k':
        kunitig_lenghts_file_given = true;
        kunitig_lenghts_file_arg = optarg;
        break;
      case 's':
        super_reads_file_given = true;
        super_reads_file_arg = optarg;
        break;
      }
    }

    // Check that required switches are present
    if(!kunitig_lenghts_file_given)
      error("[-k, --kunitig_lenghts_file=path] required switch");
    if(!super_reads_file_given)
      error("[-s, --super_reads_file=path] required switch");
    ...
    // Check access rights
    if(access(kunitig_lenghts_file_arg, R_OK)) {
      err = "Switch -k, --kunitig_lenghts_file=path, access right (read) failed for file '";
      ((err += kunitig_lenghts_file_arg) += "': ") += strerror(errno);
      error(err.c_str());
    }
    if(access(super_reads_file_arg, R_OK)) {
      err = "Switch -s, --super_reads_file=path, access right (read) failed for file '";
      ((err += super_reads_file_arg) += "': ") += strerror(errno);
      error(err.c_str());
    }
  }
  ...
  static const char * help() { return
    "Compute sizes of super-reads\n\n\n\n"
    "Options (default value in (), *required):\n"
    " -k, --kunitig_lenghts_file=path         *file with k-unitig lengths, two columns: kunitig length\n"
    " -s, --super_reads_file=path             *file with super-read names\n"
    " -U, --usage                              Usage\n"
    " -h, --help                               This message\n"
    " -V, --version                            Version";
  }
...
#define PACKAGE_VERSION "0.0.0"
  ...
  void dump(::std::ostream &os = std::cout) {
    os << "kunitig_lenghts_file_given:" << kunitig_lenghts_file_given << " kunitig_lenghts_file_arg:" << kunitig_lenghts_file_arg << "\n";
    os << "super_reads_file_given:" << super_reads_file_given << " super_reads_file_arg:" << super_reads_file_arg << "\n";
  }
};
