...
public:
  ...
  error_corrected2frg_cmdline() :
    length_min_arg((uint64_t)64), length_min_given(false),
    lib_id_arg(""),
    mean_arg(0.0),
    stdev_arg(0.0),
    number_of_reads_arg(0),
    fasta_file_arg("")
  { }
  ...
  void parse(int argc, char* argv[]) {
    ...
    static const char *short_options = "hVUl:";
...
#define CHECK_ERR(type,val,which) if(!err.empty()) { ::std::cerr << "Invalid " #type " '" << val << "' for [" which "]: " << err << "\n"; exit(1); }
    while(true) {
      ...
      case 'l':
        ...
        length_min_arg = conv_uint<uint64_t>((const char*)optarg, err, false);
    ...
    ++optind;
    mean_arg = conv_double((const char*)argv[optind], err, false);
    CHECK_ERR(double_t, argv[optind], "mean")
    ++optind;
    stdev_arg = conv_double((const char*)argv[optind], err, false);
    CHECK_ERR(double_t, argv[optind], "stdev")
    ++optind;
    number_of_reads_arg = conv_uint<uint64_t>((const char*)argv[optind], err, false);
    CHECK_ERR(uint64_t, argv[optind], "number_of_reads")
    ++optind;
    fasta_file_arg = argv[optind];
    ++optind;
  }
  static const char * usage() { return "Usage: error_corrected2frg [options] lib_id:string mean:double stdev:double number_of_reads:uint64 fasta_file:string"; }
  class error {
    int code_;
  ...
  static const char * help() { return
    "Create a frag file (.frg) from error corrected reads.\n\n\n\n"
