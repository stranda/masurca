...
public:
  ...
  cmdline_parse() :
    maximum_search_depth_arg((int)100), maximum_search_depth_given(false),
    output_arg("reduce.tmp"), output_given(false),
    largestkunitig_arg(0),
    kunitigLengthsFile_arg(""),
    kmerlen_arg(0),
    SuperReads_sizes_arg("")
  { }
  ...
  void parse(int argc, char* argv[]) {
    ...
    static const char *short_options = "hVUd:o:";
...
#define CHECK_ERR(type,val,which) if(!err.empty()) { ::std::cerr << "Invalid " #type " '" << val << "' for [" which "]: " << err << "\n"; exit(1); }
    while(true) {
      ...
      case 'd':
        ...
        maximum_search_depth_arg = conv_int<int>((const char*)optarg, err, false);
    ...
    largestkunitig_arg = conv_uint<uint64_t>((const char*)argv[optind], err, false);
    CHECK_ERR(uint64_t, argv[optind], "largestkunitig")
    ++optind;
    kunitigLengthsFile_arg = argv[optind];
    ++optind;
    kmerlen_arg = conv_int<int>((const char*)argv[optind], err, false);
    CHECK_ERR(int_t, argv[optind], "kmerlen")
    ++optind;
    SuperReads_sizes_arg = argv[optind];
    ++optind;
  }
  static const char * usage() { return "Usage: cmdline_parse [options] largestkunitig:uint64 kunitigLengthsFile:path kmerlen:int SuperReads_sizes:path"; }
  class error {
    int code_;
  ...
  static const char * help() { return
    "Reduce contained SuperReads to their largest container.\n\nLong description\n\n"
