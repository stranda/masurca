...
#define CHECK_ERR(type,val,which) if(!err.empty()) { ::std::cerr << "Invalid " #type " '" << val << "' for [" which "]: " << err << "\n"; exit(1); }
    ...
    if(!dir_given)
      error("[    --dir=path] required switch");
    if(!mer_given)
    ...
    // Check access rights
    if(access(dir_arg, R_OK)) {
      err = "Switch     --dir=path, access right (read) failed for file '";
      ((err += dir_arg) += "': ") += strerror(errno);
      error(err.c_str());
    }
  }
  ...
  static const char * help() { return
    "Expand sr_sizes file to include super-reads generated by extension of super-reads in the file.\n\n\n\n"
    "Options (default value in (), *required):\n"
    "     --dir=path                          *Directory where the super-reads work is done\n"
    " -m, --mer=int                           *Length of mer\n"
    " -U, --usage                              Usage\n"
    " -h, --help                               This message\n"
    " -V, --version                            Version";
  }
...
#define PACKAGE_VERSION "0.0.0"
  ...
  void dump(::std::ostream &os = std::cout) {
    os << "dir_given:" << dir_given << " dir_arg:" << dir_arg << "\n";
