...
#ifndef __N50_CMDLINE_HPP__
#define __N50_CMDLINE_HPP__
...
public:
  ...
  typedef ::std::vector<uint32_t>::iterator N_arg_it;
  typedef ::std::vector<uint32_t>::const_iterator N_arg_const_it;
  bool                           N_given;
  bool                           Esize_flag;
  bool                           sum_flag;
  bool                           average_flag;
  bool                           count_flag;
  bool                           from_sizes_flag;
  bool                           terse_flag;
  ...
  void parse(int argc, char* argv[]) {
    ...
    static const char *short_options = "hVUHas:N:ESACft";
...
#define CHECK_ERR(type,val,which) if(!err.empty()) { ::std::cerr << "Invalid " #type " '" << val << "' for [" which "]: " << err << "\n"; exit(1); }
    while(true) {
      ...
      case 's':
        ...
        CHECK_ERR(uint64_t, optarg, "-s, --size=N")
        break;
      case 'N':
        N_given = true;
        N_arg.push_back(conv_uint<uint32_t>((const char*)optarg, err, false));
        CHECK_ERR(uint32_t, optarg, "-N")
        break;
      case 'E':
        Esize_flag = true;
        break;
  ...
  static const char * usage() { return "Usage: n50_cmdline [options] file:string+"; }
  class error {
    int code_;
  ...
  static const char * help() { return
    "Compute statistics on fasta\n\nIf no -N or -E switch is passed, it computes the N50. Otherwise\n" \
    "compute all the statistics asked for. The E-size is always displayed\n" \
    "last.\n\n"
    "Options (default value in (), *required):\n"
    " -H, --no-header                          Do not display header (false)\n"
    " -a, --all                                All statistics and N50 if not other Nx specified (false)\n"
    " -s, --size=N                             Use N as size of genome, instead of total size in files\n"
    " -N                                       Compute Nx statistic\n"
    " -E, --Esize                              Compute the E-size (false)\n"
    " -S, --sum                                Compute sum of size of all entries (false)\n"
    " -A, --average                            Compute average length of all entries (false)\n"
    " -C, --count                              Compute the number of sequences (false)\n"
    " -f, --from-sizes                         Read list of contig sizes instead of fasta (false)\n"
    " -t, --terse                              Old terse headers (false)\n"
    " -U, --usage                              Usage\n"
    " -h, --help                               This message\n"
    " -V, --version                            Version";
  }
...
#define PACKAGE_VERSION "0.0.0"
  ...
  void dump(::std::ostream &os = std::cout) {
    ...
    os << "all_flag:" << all_flag << "\n";
    os << "size_given:" << size_given << " size_arg:" << size_arg << "\n";
    os << "N_given:" << N_given << " N_arg:" << vec_str(N_arg) << "\n";
    os << "Esize_flag:" << Esize_flag << "\n";
    os << "sum_flag:" << sum_flag << "\n";
    os << "average_flag:" << average_flag << "\n";
    os << "count_flag:" << count_flag << "\n";
    os << "from_sizes_flag:" << from_sizes_flag << "\n";
    os << "terse_flag:" << terse_flag << "\n";
    os << "file_arg:" << vec_str(file_arg) << "\n";
  }
};
#endif // __N50_CMDLINE_HPP__"
